...
<body>
	...
	<div
		...
		<table id="customers">
			...
			<jsp:useBean id="roomObject" class="com.model.Room"></jsp:useBean>
			<%
				ResultSet result = roomObject.viewBookings() ;
			%>
			<%
				while (result.next()) {
			%>
			<tr>
				<td><%=result.getInt("booking_id") %></td>
				<td><%=result.getInt("room_id") %></td>
				<td><%=result.getInt("user_id") %></td>
				<td><%=result.getDate("from_date") %></td>
				<td><%=result.getDate("to_date") %></td>
				<td><%=result.getInt("total_days")%></td>
			</tr>
			<%
				}
			%>
