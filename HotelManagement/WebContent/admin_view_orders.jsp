...
<body>
	...
	<div
		...
		<table id="customers">
			...
			<jsp:useBean id="orderObject" class="com.model.Order"></jsp:useBean>
			<%
				ResultSet result = orderObject.getAllOrders();
			%>
			<%
				while (result.next()) {
			%>
			<tr>
				...
				<td><%=result.getDouble("food_price") %></td>
				<td><%=result.getInt("food_quantity") %></td>
				<td><%=result.getDouble("total_price")%></td>
				<td><%=result.getString("ordered_time")%></td>
			</tr>
			<%
				}
			%>
